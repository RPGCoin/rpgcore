Contributing to RPGcore
=======

We're working hard to make *rpgcore* the most powerful JavaScript library for working with rpgcoin. Our goal is to have *rpgcore* be a library that can be used by anyone interested in rpgcoin, and to level expertise differences with great design and documentation.
...
These are some global design goals in rpgcore that any change must adhere.
...
When possible, rpgcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of RPGcore methods. For example, calling `JSON.stringify` on an RPGcore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'rpgcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the rpgcore object.
...
```sh
git remote add under git@github.com:RPGCoin/rpgcore.git
...
```
Finally go to [github.com/RPGCoin/rpgcore](https://github.com/RPGCoin/rpgcore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of rpgcore.

If you have any questions feel free to post them to
[github.com/RPGCoin/rpgcore/issues](https://github.com/RPGCoin/rpgcore/issues).
